...
async function demoteCommand(sock, chatId, mentionedJids, message) {
    try {
        ...
        const demotionMessage = `*『 TERMINATION LETER 』*\n\n` +
            `👤 *TERMINATED User${userToDemote.length > 1 ? 's' : ''}:*\n` +
            `${usernames.map(name => `• ${name}`).join('\n')}\n\n` +
            `👑 *TERMINATED By:* @${message.key.participant ? message.key.participant.split('@')[0] : message.key.remoteJid.split('@')[0]}\n\n` +
...
async function handleDemotionEvent(sock, groupId, participants, author) {
    try {
        ...
        const demotionMessage = `*『 TERMINATION LETER 』*\n\n` +
            `👤 *TERMINATED User${participants.length > 1 ? 's' : ''}:*\n` +
            `${demotedUsernames.map(name => `• ${name}`).join('\n')}\n\n` +
            `👑 *TERMINATED By:* ${demotedBy}\n\n` +
            `📅 *Date:* ${new Date().toLocaleString()}`;
